cc.Class({
    ...
    initPengAndGangs:function(pengangroot,side,pre,index,mjid,flag){
        ...
        if (isNaN(mjid)) {//非数字
            var paiArr=mjid.split(',');
            var max=paiArr.length;
            for(var s = 0; s < sprites.length; ++s){
                ...
                if(sprite.node.name == "gang"){
                    ...
                    if(flag == "angang"){
                        ...
                        sprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(pre,parseInt(paiArr[s]));    
                    }
                }
                else{ 
                    sprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(pre,parseInt(paiArr[s]));
            ...
            for(var s = 0; s < sprites.length; ++s){
                ...
                console.log('onPengGangChanged>>>>>',s);
                if(sprite.node.name == "gang"){
                    var isGang = flag != "peng";
                    sprite.node.active = isGang;
                    sprite.node.scaleX = 1.0;
