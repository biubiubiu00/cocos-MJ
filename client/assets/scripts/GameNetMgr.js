cc.Class({
    ...
    initHandlers:function(){
        ...
        cc.vv.net.addHandler("game_sync_push",function(data){
            ...
            for(var i = 0; i < 4; ++i){
                ...
                if(sd.teshugangs){
                    for (var j = 0; j < sd.teshugangs.length; j++) {
                        seat.diangangs.push(sd.teshugangs[j]);
                    }
                }
                //
                if(sd.zfbgangs){
                    for (var j = 0; j < sd.zfbgangs.length; j++) {
                        seat.diangangs.push(sd.zfbgangs[j]);
                    }
                }
                if(sd.fenggangs){
                    for (var j = 0; j < sd.fenggangs.length; j++) {
                        seat.diangangs.push(sd.fenggangs[j]);
                    }
                }
                if(sd.jiugangs){
                    for (var j = 0; j < sd.jiugangs.length; j++) {
                        seat.diangangs.push(sd.jiugangs[j]);
                    }
                }
                if(sd.yaogangs){
                    for (var j = 0; j < sd.yaogangs.length; j++) {
                        seat.diangangs.push(sd.yaogangs[j]);
                    }
                }
           }
        ...
        cc.vv.net.addHandler("gang_notify_push",function(data){
            ...
            var pai = data.pai;
            var si = self.getSeatIndexByID(userId);
            self.doGang(si,pai,data.gangtype,data.discardPai);
        });
    ...
    doGang:function(seatIndex,pai,gangtype,discardPai){
        ...
        if (isNaN(pai)) {//非数字
            ...
            if(discardPai){
                //加杠
                var paiArr=pai.split(',');
                var discardPaiArr=discardPai.split(',');
                for (var i = 0; i <discardPaiArr.length; i++) {
                    var newPai=discardPaiArr[i];
                    var idx = paiArr.indexOf(newPai);
                    paiArr.splice(idx,1);
                }
                //去掉加杠涉及的手牌
                if(seatData.holds){
                    for (var i = 0; i <paiArr.length; i++) {
                        var newPai=paiArr[i];
                        var idx = seatData.holds.indexOf(parseInt(newPai));
                        if(idx != -1){
                            seatData.holds.splice(idx,1);
                        }
                    }
                ...
                if(seatData.diangangs){
                    var idx = seatData.diangangs.indexOf(discardPai);
                    if(idx != -1){
                        seatData.diangangs.splice(idx,1);
                    }
                }
                console.log('doGang>>>加杠 >>> ',seatData.diangangs,seatData.holds);
            }else{
                var paiArr=pai.split(',');
                if(seatData.holds){
                    for (var i = 0; i <paiArr.length; i++) {
                ...
                console.log('doGang >>> ',seatData.holds);
            }
            //
            seatData.diangangs.push(pai);
            this.dispatchEvent('gang_notify',{seatData:seatData,gangtype:gangtype,newGang:true});
        }else{
            if(!gangtype){
